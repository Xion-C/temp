...
def read_images(base_dir):
    ...
    for dir_name in ['train', 'val']:
        ...
        print('- reading images in {}: {}'.format(data_dir, len(paths)))
...
def store_h5py(base_dir, dir_name, images, image_size):
    file_path = os.path.join(base_dir, '{}_{}.hy'.format(dir_name, image_size))
...
def read_h5py(task_name, image_size):
    ...
    ret = []
    # print('start reading h5py files')
    for dir_name in ['trainA', 'trainB', 'valA', 'valB']:
        ...
        print('- {} h5py file loaded'.format(dir_name))
...
def get_data(path, task_name, image_size):
